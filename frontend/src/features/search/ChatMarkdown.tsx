  ...
  for (const match of text.matchAll(pattern)) {
    ...
    nodes.push(
      <Link
        key={`${key}-${count++}`}
        to={link?.href ?? '#'}
        className="font-medium text-ink underline decoration-accent-strong/70 decoration-[1.5px] underline-offset-2 transition-colors duration-150 ease-brand hover:decoration-accent-strong"
      >
        {match[0]}
      </Link>,
    )
