...
move support tickets. Views validate and check permissions; ``TicketService``
...
@extend_schema_view(
    list=extend_schema(
        ...
        description=(
            "HR admins see every ticket; everyone else sees the tickets they raised or "
            "were assigned. `search` matches the number, subject and description."
        ),
        parameters=[
    ...
    create=extend_schema(
        ...
        summary="Raise a ticket (everyone); the support team is notified",
...
class TicketViewSet(
    ...
    def _detail(self, ticket: Ticket, code: int = status.HTTP_200_OK) -> Response:
        row = self.get_queryset().prefetch_related("events__actor").get(pk=ticket.pk)
        return Response(
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        ticket = TicketService.create(requester=request.user, **data)
    ...
    @extend_schema(
        ...
        summary="Add a comment to the ticket's timeline",
    ...
    def comments(self, request: Request, pk: str | None = None) -> Response:
        ...
        serializer.is_valid(raise_exception=True)
        TicketService.comment(ticket, request.user, serializer.validated_data["message"])
        return self._detail(ticket)
