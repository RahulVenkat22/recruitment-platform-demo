...
| `pipeline` (outreach) | `MessageTemplate`, `OutreachService` (`pipeline/services/outreach.py`): template rendering, safe-mode redirect, SMTP send via Django's email backend, logging through `CommunicationService` (`email_sent` ⇒ Contact Pending); `GET /email/`, `email/templates/` CRUD (HR staff write) with `POST email/templates/generate/` (AI draft via the search model, `draft_email`), `POST /applications/{id}/email/preview/`, `POST /applications/{id}/email/`, `POST /applications/bulk-email/` (template or raw text with placeholders) |
| `pipeline` (AI calls) | `PhoneCall`, `pipeline/services/calls.py` (`build_agent_prompt`, `next_turn`, `assess`, `CallService.create/reply/finish/apply_event`), `pipeline/services/voice.py` (`normalize_number` via phonenumbers, `VapiProvider`, safe mode); `POST /applications/{id}/calls/`, `POST /applications/bulk-calls/`, `calls/` list/detail, `calls/{id}/reply|finish/`, `calls/config/`, `POST calls/webhook/vapi/` (shared secret, no session) |
| `pipeline` (results chat) | `SearchChatMessage` (one turn of a user's private conversation about one search run, with the candidates an answer cites) and `pipeline/services/chat.py::SearchChatService`: `thread` (the scope, the turns, opening questions built from the real results), `ask` (the answer as server-sent events: `context`, `token`…, `done` or `error`; the question is stored only once the answer is) and `clear`. The assistant is grounded in that run only: the job brief the run analysed, the search facts, the ranked results (compact rows for the top 40, full cards for the top 10 and anyone the question names), and the resume chunks closest to the question (pgvector, within the result set). `GET/POST/DELETE /searches/{id}/chat/`; `SEARCH_CHAT_MODEL` (default: the search model) |
| `sourcing` | `CandidateSourceProvider` ABC, DTOs, provider registry, the four database-backed providers (the internal one also runs the semantic search over ingested resumes), `SearchService` (phased, background-capable) |
...
- Motion uses 450ms page entrances, 280ms dialogs, staggered list entrances and animated charts, counters and progress indicators. `useMotionPreference` combines the browser-persisted setting with live system reduced-motion changes; CSS effects follow the same preference. See [UI experience](docs/ui-experience.md).
- Visual system: deep navy navigation, teal actions, cool white surfaces and the Buro Happold lime accent, with Manrope headings and Geist UI text. Tokens live in `index.css`; shared interaction styling lives in `experience.css`. `data-surface="dark"` re-points the tokens for dark panels. Existing logo assets remain in `public/brand/` behind `components/shared/BrandLogo`.
