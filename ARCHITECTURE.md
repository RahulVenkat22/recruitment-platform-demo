...
| `pipeline` (outreach) | `MessageTemplate`, `OutreachService` (`pipeline/services/outreach.py`): template rendering, safe-mode redirect, SMTP send via Django's email backend, logging through `CommunicationService` (`email_sent` ⇒ Contact Pending); `GET /email/`, `POST /applications/{id}/email/preview/`, `POST /applications/{id}/email/`, `POST /applications/bulk-email/` |
